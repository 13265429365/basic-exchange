<template>
  <q-list class="q-ml-xl q-mt-xl q-mr-md" style="min-width: 220px;width: 220px" padding>
    <div v-for="(menu, menuIndex) in menuList" :key="menuIndex">
      <!-- 含有子级的列表 -->
      <q-expansion-item accordion :header-style="{ borderRadius: '8px' }" v-if="menu.children.length > 0" default-opened>
        <!-- 父级 -->
        ...
        <!-- 子级 -->
        <q-item class="radius-8" :to="children.route" v-for="(children, childrenIndex) in menu.children"
      ...
      <!-- 不含子级的列表 -->
      <q-item v-else class="radius-8" :active="$route.path == menu.route" :to="menu.route">
        <q-item-section avatar>
          <q-img :src="imageSrc(menu.icon)" width="24px" height="24px"></q-img>
        </q-item-section>
        <q-item-section>
