<template>
  <q-list padding class="q-ml-xl q-mt-xl" style="width: 220px">
      ...
      <q-expansion-item v-if="item.child" default-opened>
        ...
        <template v-slot:header>
          <q-item-section avatar>
            <q-img :src="`/images/pc/menu/${activeRouter == item.url ? 'act_' + item.icon : item.icon
              }.png`" width="24px" height="24px"></q-img>
          </q-item-section>
        ...
        <!-- 子级 -->
        <q-item @click="to(str.url)" v-for="(str, s) in item.child" :key="s" :header-inset-level="1"
          :active="activeRouter == str.url" active-class="active" clickable v-ripple>
