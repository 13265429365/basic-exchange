        ...
        <q-card-section class="q-pt-none">
          <div class="relative row justify-center" v-if="currentSetting.params == 'avatar'">
            <q-avatar style="width: 80px;height: 80px;">
              ...
              <uploader type="avatar" @uploaded="(url) => {params[currentSetting.params] = url;currentSetting.value = url}"
          ...
          <div v-else-if="currentSetting.params == 'password' || currentSetting.params == 'secretKey'">
            <q-input v-model="params.oldPassword" outlined dense class="q-mb-md" :type="showPwd.oldPwd ? 'text' : 'password'" :label="$t('oldPassword')">
              <template v-slot:append>
                <q-icon @click="showPwd.oldPwd = !showPwd.oldPwd" :name="showPwd.oldPwd ? 'o_visibility' : 'o_visibility_off'"></q-icon>
              </template>
            </q-input>
            <q-input v-model="params.newPassword" outlined dense class="q-mb-md" :type="showPwd.newPwd ? 'text' : 'password'" :label="$t('newPassword')">
              <template v-slot:append>
                <q-icon @click="showPwd.newPwd = !showPwd.newPwd"
                  :name="showPwd.newPwd ? 'o_visibility' : 'o_visibility_off'"></q-icon>
              </template>
            </q-input>
            <q-input v-model="params.cmfPassword" outlined dense class="q-mb-md" :type="showPwd.cmfPwd ? 'text' : 'password'" :label="$t('cmfPassword')">
              <template v-slot:append>
          ...
          <div v-else-if="currentSetting.params == 'telephone'">
            <div class="row no-wrap justify-between">
              ...
              <div class="col-8">
                <q-input dense class="q-ml-xs" outlined v-model="params[currentSetting.params]" :label="$t('telephone')" />
              </div>
            </div>
          </div>

          <div v-else-if="currentSetting.params == 'desc'">
            <q-input outlined dense type="textarea" v-model="params[currentSetting.params]" :label="$t(currentSetting.name)"></q-input>
          </div>
